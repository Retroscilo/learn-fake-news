...
const isLineBeginning = (names, string) => {
	return string.split(" – ")[1] && names.some(name => string.split(" – ")[0].includes(name));
};
...
export function characterNames(full = false) {
	...
	lines.map(line => {
		...
		if (_line.length < 2 || names.indexOf(_line[0]) !== -1 || (!full && _line[0].match(/\(|\)/g))) return;
		names.push(_line[0]);
	});
	return names;
}
...
export function getLines(character) {
	...
	lines.map((line, i) => {
		...
		if (isLineBeginning(names, line) && isChar(names, character, line)) {
			...
			let above = lines[i - 1];
			let backward = 1;
			while (lines[i - backward - 1] && !isLineBeginning(names, lines[i - backward])) {
		...
		if (!isLineBeginning(names, line)) {
			...
			do {
				backward++;
				if ((lines[i - backward] && !isLineBeginning(names, lines[i - backward])) || !charLines[i - backward]) continue;
				charLines[i - backward].line += " " + line.trim();
			} while (lines[i - backward] && !isLineBeginning(names, lines[i - backward]));
		}
	});

	// reset keys to be 0...n
	return Object.values(charLines).reduce((prev, value, i) => ({ ...prev, [i]: value }), {});
}
